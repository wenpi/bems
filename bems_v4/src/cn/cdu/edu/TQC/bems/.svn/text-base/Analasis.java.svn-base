...
public class Analasis {
	...
	public List<Double> getDeviceByMonth(String deviceType, String deviceID,
		...
		for (int i = 1; i <= month; i++) {
			...
			Double d = getAmongData(deviceType, deviceID, begin, tempDate);
			data.add(d);
			begin = tempDate;
		}
		return data;
	...
	public List<Double> getDevicesAtOneTime(String deviceType, int year,
		...
		if (deviceType.endsWith("meter")) {
			...
			for (Meter meter : meters) {
				if (meter != null) {
					Double d = getAmongData(deviceType, meter.getMeterId(),
							begin, end);
					all.add(d);
				}
			}
		...
		if (deviceType.equals("ammeter")) {
			...
			for (int i = 0; i < ammeters.size(); i++) {
				...
				if (ammeter != null && ammeter.getAmmeterId() != null) {
					Double d = getAmongData(deviceType, ammeter.getAmmeterId(),
							begin, end);
					all.add(d);
	...
	public static void main(String[] args) {
		...
		// List<Device> devices = new Analasis().getDevicesData("ammeter",ids ,
		// new Date("2010/1/1"),new Date("2010/12/30"));
		// for(Device dd : devices){ 20120001 20000002
		// System.out.println("--------"+dd.getName());
		// System.out.println("---------"+dd.getData().length);
		// }
		// List<Double> all = new Analasis().getDeviceByMonth("ammeter", ids[1],
		// 2010);
		// for (Double d : all) {
		...
		// List<Double> alls = new Analasis().getDeviceBySeason("ammeter",
		// ids[1],
		// 2010);
		// for (Double d : all) {
		// System.out.println("----" + d);
		...
		// System.out.println("========="+Utils.getOffsetYearDate(new
		// Date("2010/12/1"), 30).toLocaleString());
	}
